...
def GetAuthorized(firebase):
    ...
    # authenticate a user
    try:
        user = auth.sign_in_with_email_and_password("ayazingar@gmail.com",
                                                   "ayaz123")  # username and password of your account for database
        #user = auth.sign_in_with_email_and_password("testdemo@gmail.com",
                                                  #  "testdemo")  # username and password of your account for database
...
def sendtoUserFirebase(db, sensordata):
    ...
    result = db.child("users").child(userid).push(sensordata, user["idToken"])  # needs the authorization to save the data
    print(result)
    return;

# The function to set up the record structure to be written to the database.
# ====================================================================================================
def setupData( temperature, pressure, humidity, motionDet, timestamp):
...
def main():
  ...
  while (1):
      ...
      #motionDet = pir.wait_for_motion()
      #print("Motion Detected")
 
      sensordata = setupData(temperature,pressure,humidity,motionDet,int(time.time()))
      #sendtoFirebase(firebase.database(), sensordata)
      sendtoUserFirebase(firebase.database(), sensordata)
      ...
      if (count == 5):    # exit the program after 10 readings.
        break;
   
      GetDatafromFirebase(firebase.database())
